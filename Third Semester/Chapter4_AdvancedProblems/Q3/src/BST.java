...
public class BST {	
	...
	public void getLinks(Node x)
	{
		...
		if(x==null)
			return;
		if(x.left!=null&&(x.left.size==total/3||x.left.size==total/3-1)&&counter<2)
		{
			System.out.println("Link Break between="+x.left.key+" and "+x.key);
			x.left=null;
			counter++;
			initializeSubtreeSize(root);
			getLinks(root);
		}
		if(x.right!=null&&(x.right.size==total/3||x.right.size==total/3-1)&&counter<2)
		{
